...
class SyntheticVideoDataset(Dataset):
    ...
    def __init__(self, parameters, *, size=384, seq_length=3, max_num_obj=1):
        ...
        self.frame_image_dual_transform = transforms.Compose(
            [
                transforms.RandomAffine(
                    degrees=20, scale=(0.5, 2.0), shear=10, interpolation=InterpolationMode.BILINEAR, fill=im_mean
                ),
        ...
        self.frame_mask_dual_transform = transforms.Compose(
            [
                transforms.RandomAffine(
                    degrees=20, scale=(0.5, 2.0), shear=10, interpolation=InterpolationMode.NEAREST, fill=0
                ),
