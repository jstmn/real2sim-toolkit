...
class InferenceCore:
    ...
    def step(
        ...
        *,
            ...
            if min_side > self.max_internal_size:
                ...
                if mask is not None:
                    if idx_mask:
                        mask = (
                            F.interpolate(
                                mask.unsqueeze(0).unsqueeze(0).float(), size=(new_h, new_w), mode="nearest-exact"
                            )[0, 0]
                            .round()
                            .long()
                        )
        ...
        if is_mem_frame or force_permanent:
            self._add_memory(
                image, pix_feat, self.last_mask, key, shrinkage, selection, force_permanent=force_permanent
            )
    ...
    def output_prob_to_mask(self, output_prob: torch.Tensor, segment_threshold: float = 0) -> torch.Tensor:
        output_prob[1] = output_prob[1] - segment_threshold
