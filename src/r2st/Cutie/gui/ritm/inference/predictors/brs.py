    ...
    def _get_prediction(self, image_nd, clicks_lists, is_image_changed):
        ...
        def get_prediction_logits(scale, bias):
            ...
            if self.insertion_mode == "after_c4":
                ...
                x = F.interpolate(x, mode="bilinear", size=self._c1_features.size()[2:], align_corners=True)
        ...
        if num_clicks > self.optimize_after_n_clicks:
            opt_result = fmin_l_bfgs_b(func=self.opt_functor, x0=self.opt_data, **self.opt_functor.optimizer_params)
    ...
    def _get_prediction(self, image_nd, clicks_lists, is_image_changed):
        ...
        if self.opt_data is None or is_image_changed:
            ...
            self.opt_data = torch.zeros(
                (bs, opt_channels, image_nd.shape[2], image_nd.shape[3]), device=self.device, dtype=torch.float32
            )
        ...
        if num_clicks > self.optimize_after_n_clicks:
            opt_result = fmin_l_bfgs_b(
                func=self.opt_functor, x0=self.opt_data.cpu().numpy().ravel(), **self.opt_functor.optimizer_params
            )
