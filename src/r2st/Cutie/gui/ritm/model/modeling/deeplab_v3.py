...
class DeepLabV3Plus(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.backbone = ResNetBackbone(
            backbone=self.backbone_name, pretrained_base=False, norm_layer=self.backbone_norm_layer, **kwargs
        )
    ...
    def load_pretrained_weights(self):
        pretrained = ResNetBackbone(
            backbone=self.backbone_name, pretrained_base=True, norm_layer=self.backbone_norm_layer, **self._kwargs
        )
...
class _SkipProject(nn.Module):
    def __init__(self, in_channels, out_channels, norm_layer=nn.BatchNorm2d):
        ...
        self.skip_project = nn.Sequential(
            nn.Conv2d(in_channels, out_channels, kernel_size=1, bias=False), norm_layer(out_channels), _activation()
        )
