...
from ..model.ops import BatchImageNormalize, DistMaps, ScaleLayer
        ...
        if self.clicks_groups is not None:
            ...
            for click_radius in self.clicks_groups:
                self.dist_maps.append(
                    DistMaps(norm_radius=click_radius, spatial_scale=1.0, cpu_mode=cpu_dist_maps, use_disks=use_disks)
                )
        else:
