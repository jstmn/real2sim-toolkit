...
class MainController:
    ...
    def __init__(self, cfg: DictConfig) -> None:
        ...
        if cfg.get("workspace_init_only"):
            return
    ...
    def compose_current_im(self):
        self.vis_image = get_visualization(
            self.vis_mode, self.curr_image_np, self.curr_mask, self.overlay_layer, self.vis_target_objects
        )
    ...
    def update_current_image_fast(self, invalid_soft_mask: bool = False):
        ...
        self.vis_image = get_visualization_torch(
            self.vis_mode, self.curr_image_torch, self.curr_prob, self.overlay_layer_torch, self.vis_target_objects
        )
        self.curr_image_torch = None
        self.vis_image = np.ascontiguousarray(self.vis_image)
        save_visualization = self.save_visualization_mode in ["Propagation only (higher quality)", "Always"]
    ...
    def on_export_binary(self):
        ...
        if path.exists(mask_folder):
            ...
            convert_mask_to_binary(
                mask_folder, save_folder, self.vis_target_objects, progress_callback=self.gui.progressbar_update
            )
