...
def sample_rays_uniform(N_samples, near, far, lindisp=False, perturb=True):
    ...
    """
    N_ray = near.shape[0]
    t_vals = torch.linspace(0.0, 1.0, steps=N_samples, device=near.device).reshape(1, -1)
    if not lindisp:
        z_vals = near * (1.0 - t_vals) + far * (t_vals)
    else:
        z_vals = 1.0 / (1.0 / near * (1.0 - t_vals) + 1.0 / far * (t_vals))  # (N_ray,N_sample)
...
class NerfRunner:
    def __init__(
        self, cfg, images, depths, masks, normal_maps, poses, K, _run=None, occ_masks=None, build_octree_pcd=None
    ):
        set_seed(0)
        self.cfg = cfg
        ...
        if self.cfg["denoise_depth_use_octree_cloud"]:
            ...
            logging.info("Denoising rays based on octree cloud")
    ...
    def create_nerf(self, device=torch.device("cuda")):
        ...
        if self.cfg["use_viewdirs"]:
            embeddirs_fn, input_ch_views = get_embedder(
                self.cfg["multires_views"], self.cfg, i=self.cfg["i_embed_views"], octree_m=self.octree_m
            )
    ...
    def create_optimizer(self):
        ...
        if self.models["pose_array"] is not None:
            param_groups.append(
                {"name": "pose_array", "params": self.models["pose_array"].parameters(), "lr": self.cfg["lrate_pose"]}
            )
    ...
    def get_truncation(self):
        ...
        if self.cfg["trunc_decay_type"] == "linear":
            truncation = (
                ...
                - (self.cfg["trunc_start"] - self.cfg["trunc"]) * float(self.global_step) / self.cfg["n_step"]
            )
    ...
    def train_loop(self, batch):
        ...
        fs_loss, sdf_loss, empty_loss, front_mask, sdf_mask = get_sdf_loss(
            z_vals,
        ...
        if self.cfg["fs_rgb_weight"] > 0:
            fs_rgb_loss = (
                (((torch.sigmoid(extras["raw"][..., :3]) - 1) * front_mask[..., None]) ** 2) * sample_weights[..., None]
            ).mean()
        ...
        if self.global_step % self.cfg["i_weights"] == 0 and self.global_step > 0:
            self.save_weights(out_file=os.path.join(self.cfg["save_dir"], "model_latest.pth"), models=self.models)
            ...
            with torch.no_grad():
                ...
                if mesh is not None:
                    ...
                    mesh = mesh_to_real_world(
                        mesh, offset, translation=self.cfg["translation"], sc_factor=self.cfg["sc_factor"]
                    )
                    mesh.export(dir)
                    if self._run is not None:
                        self._run.add_artifact(dir)
    ...
    def sample_rays_uniform_occupied_voxels(
        self, rays_d, depths_in_out, lindisp=False, perturb=False, depths=None, N_samples=None
    ):
    ...
    def render_rays(
        ...
        if self.cfg["N_samples_around_depth"] > 0 and depth is not None:  #!NOTE only fine when depths are all valid
            ...
            if invalid_depth_mask.any() and self.cfg["use_octree"]:
                z_vals_invalid, _ = self.sample_rays_uniform_occupied_voxels(
                    rays_d=viewdirs[invalid_depth_mask],
                    depths_in_out=depths_in_out[invalid_depth_mask],
                    lindisp=lindisp,
                    perturb=perturb,
                    depths=None,
                ...
                z_vals_around_depth[invalid_depth_mask] = sample_rays_uniform(
                    self.cfg["N_samples_around_depth"],
        ...
        raw, normals, valid_samples = self.run_network(
            pts, viewdirs, frame_ids, tf=tf, valid_samples=valid_samples, get_normals=get_normals
        )  # [N_rays, N_samples, 4]

        rgb_map, weights = self.raw2outputs(
            raw, z_vals, rays_d, raw_noise_std=raw_noise_std, valid_samples=valid_samples, depth=depth
        )
        ...
        if self.cfg["N_importance"] > 0:
            ...
            for iter in range(self.cfg["N_importance_iter"]):
                ...
                z_samples = sample_pdf(z_vals_mid, weights[..., 1:-1], self.cfg["N_importance"], det=(perturb == 0.0))
                ...
                if self.models["model_fine"] is not None and self.models["model_fine"] != self.models["model"]:
                    ...
                    raw, normals, valid_samples = self.run_network(
                        pts, viewdirs, frame_ids, tf=tf, valid_samples=valid_samples, get_normals=False
                    )
                else:
                    pts = (
                    ...
                    raw_fine, valid_samples_importance = self.run_network(
                        pts, viewdirs, frame_ids, tf=tf, valid_samples=valid_samples_importance, get_normals=False
                    )
                    ...
                    raw = torch.gather(
                        torch.cat([raw, raw_fine], dim=1), dim=1, index=indices[..., None].expand(-1, -1, raw.shape[-1])
                    )
                    valid_samples = torch.cat((valid_samples, valid_samples_importance), dim=-1)

                rgb_map, weights = self.raw2outputs(
                    raw, z_vals, rays_d, raw_noise_std=raw_noise_std, valid_samples=valid_samples
                )
    ...
    def raw2outputs(self, raw, z_vals, rays_d, raw_noise_std=0, valid_samples=None, depth=None):
        ...
        def sdf2weights(sdf):
            ...
            weights = torch.sigmoid(sdf_from_depth * self.cfg["sdf_lambda"]) * torch.sigmoid(
    ...
    def render(self, rays, depth=None, lindisp=False, perturb=False, raw_noise_std=0.0, get_normals=False):
        ...
        all_ret = self.batchify_rays(
            rays, depth=depth, lindisp=lindisp, perturb=perturb, raw_noise_std=raw_noise_std, get_normals=get_normals
        )
    ...
    def run_network(
        self, inputs, viewdirs, frame_ids=None, tf=None, latent_code=None, valid_samples=None, get_normals=False
    ):
        ...
        with torch.cuda.amp.autocast(enabled=self.cfg["amp"]):
            if self.cfg["i_embed"] in [3]:
                embedded[valid_samples.reshape(-1)], valid_samples_embed = self.models["embed_fn"](
    ...
    def extract_mesh(self, level=None, voxel_size=0.003, isolevel=0.0, return_sigma=False):
        ...
        N = len(tx)
        query_pts = (
            torch.tensor(np.stack(np.meshgrid(tx, ty, tz, indexing="ij"), -1).astype(np.float32).reshape(-1, 3))
            .float()
            .cuda()
        )
    ...
    def mesh_texture_from_train_images(self, mesh, rgbs_raw, tex_res=1024):
        ...
        renderer = ModelRendererOffscreen(
            cam_K=self.K, H=self.H, W=self.W, zfar=self.cfg["far"] * self.cfg["sc_factor"]
        )
        ...
        logging.info("Texture: Texture map computation")
        for i in range(len(rgbs_raw)):
            ...
            tex_image[uvs_unique[:, 1], uvs_unique[:, 0]] += torch.from_numpy(ray_colors).cuda().float()[
                unique_ids
            ] * cur_weights.reshape(-1, 1)
