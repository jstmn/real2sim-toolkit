...
def run_neural_object_field(
    cfg, K, rgbs, depths, masks, cam_in_obs, debug=0, save_dir="/home/bowen/debug/foundationpose_bundlesdf"
):
    ...
    nerf = NerfRunner(
        cfg, rgbs_, depths_, masks_, normal_maps=None, poses=poses, K=K, occ_masks=None, build_octree_pcd=pcd_normalized
    )
    nerf.train()
    ...
    mesh = mesh_to_real_world(
        mesh, pose_offset=offset, translation=nerf.cfg["translation"], sc_factor=nerf.cfg["sc_factor"]
    )
    return mesh
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dataset", type=str, default="ycbv", help="one of [ycbv/linemod]")
