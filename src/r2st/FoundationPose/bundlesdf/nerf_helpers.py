...
class FeatureArray(nn.Module):
    ...
    def __init__(self, num_frames, num_channels):
        ...
        self.data = nn.parameter.Parameter(
            torch.normal(0, 1, size=[num_frames, num_channels]).float(), requires_grad=True
        )
...
def get_sdf_loss(z_vals, target_d, predicted_sdf, truncation, cfg, return_mask=False, sample_weights=None, rays_d=None):
    ...
    sdf_loss = (
        torch.mean(((z_vals + predicted_sdf * truncation) * sdf_mask - target_d * sdf_mask) ** 2 * sample_weights)
        * sdf_weight
    )
...
def ray_box_intersection_batch(origins, dirs, bounds):
    ...
    tmin = (
        torch.gather(bounds[..., 0], dim=1, index=sign[:, 0].reshape(-1, 1)).reshape(-1) - origins[:, 0]
    ) * inv_dirs[
        :, 0
    ]  # (N)
    tmin[tmin < 0] = 0
    tmax = (
        torch.gather(bounds[..., 0], dim=1, index=1 - sign[:, 0].reshape(-1, 1)).reshape(-1) - origins[:, 0]
    ) * inv_dirs[:, 0]
    tymin = (
        torch.gather(bounds[..., 1], dim=1, index=sign[:, 1].reshape(-1, 1)).reshape(-1) - origins[:, 1]
    ) * inv_dirs[:, 1]
    tymin[tymin < 0] = 0
    tymax = (
        torch.gather(bounds[..., 1], dim=1, index=1 - sign[:, 1].reshape(-1, 1)).reshape(-1) - origins[:, 1]
    ) * inv_dirs[:, 1]
    ...
    tzmin = (
        torch.gather(bounds[..., 2], dim=1, index=sign[:, 2].reshape(-1, 1)).reshape(-1) - origins[:, 2]
    ) * inv_dirs[:, 2]
    tzmin[tzmin < 0] = 0
    tzmax = (
        torch.gather(bounds[..., 2], dim=1, index=1 - sign[:, 2].reshape(-1, 1)).reshape(-1) - origins[:, 2]
    ) * inv_dirs[:, 2]
