...
from r2st.core import SAM2Predictor, bbox_xyxy_from_mask, binary_mask_to_sam_mask_input
...
class Args:
    ...
    sam_kmax: int = 5
    """Union at most this many highest-confidence SAM 2 masks into the frame-0 seed."""

    sam_score_threshold: float = 0.3
    """Keep a top-k SAM 2 mask in the frame-0 union only if its confidence is strictly above this."""
...
def _union_top_sam_masks(
    ...
    n_keep = int(keep.sum())
    assert n_keep >= 1, f"No SAM 2 masks in top {n} with score > {score_threshold} (scores={scores[:n].tolist()})"
...
def _propagate_masks(rgb: np.ndarray, predictor: SAM2Predictor, seed_mask: np.ndarray) -> np.ndarray:
